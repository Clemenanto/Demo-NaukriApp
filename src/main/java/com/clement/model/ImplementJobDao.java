...
import com.clement.configuration.JDBCConnection;
...
public class ImplementJobDao implements JobDao {
		...
		public void updateJob(Job job) {
			try {
				...
				if(i==1){
					...
					JOptionPane.showMessageDialog(frame,"No Job Id Found");
				}
			}
			catch(SQLException ex) {
				System.out.println(ex.getMessage());
		...
		public void deactivateJob(Job job) {
			try {
				...
				JFrame frame=new JFrame();
				if(i==1){
					JOptionPane.showMessageDialog(frame, "Job Deactivated successfully");
